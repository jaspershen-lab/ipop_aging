...
dir.create("data_analysis/plasma_metabolomics/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("data_analysis/plasma_metabolomics/linear_spearman/cross_section")
...
#                spearman_cor = lm_result$estimate[2])
#   }) %>%
#   dplyr::bind_rows() %>%
#   as.data.frame()
...
plot <-
  ...
  scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(n = 11, name = "Spectral"))) +
  ggrepel::geom_text_repel(aes(label = adjusted_age), size = 5) +
  theme_base
...
variable_info <-
  ...
  dplyr::left_join(variable_info[,c("variable_id", "Compound.name")],
                   by = "variable_id")
