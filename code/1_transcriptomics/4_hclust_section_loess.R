...
dir.create("data_analysis/plasma_transcriptome/hclust/cross_section_loess/",
           recursive = TRUE)

setwd("data_analysis/plasma_transcriptome/hclust/cross_section_loess")
...
unique(hc_cluster) %>%
  purrr::map(function(i) {
    ...
    temp %>%
      ggplot(aes(adjusted_age, value)) +
      geom_line(aes(group = variable_id))
...
library(dendextend)
hc <- as.dendrogram(hc)
# hc <- color_branches(hc, h = 3)
plot(hc, leaflab = "none")
rect.hclust(hc , k = 3)

plot(as.dendrogram(hc), label = FALSE)
