...
temp_data_skin_microbiome <-
  temp_data %>%
  dplyr::filter(class == "skin_microbiome" & p_value_adjust < 0.05)

####skin_microbiome
dir.create("skin_microbiome_pathway_crest1")

####crest 1
temp_data_skin_microbiome_crest1 <-
  temp_data_skin_microbiome %>%
  dplyr::filter(center == 43)

dim(temp_data_skin_microbiome_crest1)

length(unique(temp_data_skin_microbiome$variable_id))

library(openxlsx)
wb <- createWorkbook()
modifyBaseFont(wb, fontSize = 12, fontName = "Times New Roma")
addWorksheet(wb, sheetName = "skin_microbiomes in crest1", gridLines = TRUE)
freezePane(wb,
           sheet = 1,
           firstRow = TRUE,
           firstCol = TRUE)
writeDataTable(
  wb,
  sheet = 1,
  x = temp_data_skin_microbiome_crest1 %>% dplyr::select(-Class),
  colNames = TRUE,
  rowNames = FALSE
)
saveWorkbook(
  wb,
  "skin_microbiome_pathway_crest1/skin_microbiome_makrers.xlsx",
  overwrite = TRUE
)
