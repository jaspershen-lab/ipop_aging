...
temp_data_skin_microbiome <-
  temp_data %>%
  dplyr::filter(class == "skin_microbiome" &
                  p_value_adjust < 0.05)

temp_data_oral_microbiome <-
  temp_data %>%
  dplyr::filter(class == "oral_microbiome" &
                  p_value_adjust < 0.05)

temp_data_nasal_microbiome <-
  temp_data %>%
  dplyr::filter(class == "nasal_microbiome" &
                  p_value_adjust < 0.05)

####transcriptome
dir.create("transcriptome_pathway")
dir.create("transcriptome_pathway/GO_result/")
dir.create("transcriptome_pathway/KEGG_result/")
dir.create("transcriptome_pathway/Reactome_result/")
...
#   enrichGO(
#     gene = temp_data_transcriptome_crest1$ENTREZID[!is.na(temp_data_transcriptome_crest1$ENTREZID)],
#     OrgDb = org.Hs.eg.db,
#     keyType = "ENTREZID",
#     ont = "ALL",
#     pvalueCutoff = 0.05,
...
# save(transcriptome_crest1_go, file = "transcriptome_pathway/GO_result/transcriptome_crest1_go")
...
#   enrichKEGG(
#     gene = temp_data_transcriptome_crest1$UNIPROT[!is.na(temp_data_transcriptome_crest1$UNIPROT)],
#     organism = "hsa",
#     keyType = "uniprot",
#     pvalueCutoff = 0.05,
#     pAdjustMethod = "fdr",
...
#   dplyr::filter(p.adjust < 0.05)
# 
# result <- transcriptome_crest1_kegg@result
# 
# kegg_class <-
#   purrr::map(
#     .x = result$ID,
...
#   unlist()
# 
# transcriptome_crest1_kegg@result$kegg_class <- kegg_class
# 
# save(transcriptome_crest1_kegg, file = "transcriptome_pathway/KEGG_result/transcriptome_crest1_kegg")
load("transcriptome_pathway/KEGG_result/transcriptome_crest1_kegg")

# ##Reactome pathway enrichment
# library(ReactomePA)
# transcriptome_crest1_reactome <-
#   ReactomePA::enrichPathway(gene = temp_data_transcriptome_crest1$ENTREZID[!is.na(temp_data_transcriptome_crest1$ENTREZID)],
#                             organism = "human",
#                             pvalueCutoff = 0.05,
#                             pAdjustMethod = "fdr",
#                             qvalueCutoff = 0.05)
...
# disease <-
#   purrr::map(transcriptome_crest1_reactome@result$ID, function(x) {
...
#   unlist()
# 
# transcriptome_crest1_reactome@result$disease <-
#   disease
# 
# save(transcriptome_crest1_reactome, file = "transcriptome_pathway/Reactome_result/transcriptome_crest1_reactome")
...
# go_sim_matrix =
#   get_go_result_sim(result = transcriptome_crest1_go, sim.cutoff = 0)
# 
# save(go_sim_matrix, file = "transcriptome_pathway/GO_result/go_sim_matrix")

load("transcriptome_pathway/GO_result/go_sim_matrix")
...
#                                                            "sim")))
# 
# save(subnetwork, file = "transcriptome_pathway/GO_result/subnetwork")
load("transcriptome_pathway/GO_result/subnetwork")
...
# )
# 
# saveWorkbook(wb, "transcriptome_pathway/GO_result/result_go.xlsx", overwrite = TRUE)
...
# ggplot2::ggsave(plot,
#        filename =
#          "transcriptome_pathway/GO_result/GO_sim_plot.pdf",
#        width = 7,
#        height = 7)

dir.create("transcriptome_pathway/GO_result/GO_sim_plot")
...
# )
# 
# saveWorkbook(wb, "transcriptome_pathway/GO_result/result_go_cluster.xlsx", overwrite = TRUE)

###output the cluster annotation for each cluster
dir.create("transcriptome_pathway/GO_result/GO_graph")
...
# ancestor <- GOSim::getAncestors()
# parents <- GOSim::getParents()
# children <- GOSim::getChildren()
# offstrings <- GOSim::getOffsprings()

#       parent2 <-
#         try(GOfuncR::get_parent_nodes(go_ids = x[1]), silent = TRUE)
#       children2 <- try(GOfuncR::get_child_nodes(go_ids = x[1]))
...
# kegg_sim_matrix <-
#   simplifyEnrichment::term_similarity_from_KEGG(term_id = c(transcriptome_crest1_kegg$ID),
#                                                 method = "jaccard") %>%
#   as.data.frame() %>%
...
# 
# save(kegg_sim_matrix, file = "transcriptome_pathway/KEGG_result/kegg_sim_matrix")

load("transcriptome_pathway/KEGG_result/kegg_sim_matrix")
...
node_data <-
  rbind(transcriptome_crest1_kegg) %>%
  as.data.frame() %>%
  dplyr::select(ID, everything()) %>%
  dplyr::rename(node = ID)
...
# save(result_kegg, file = "transcriptome_pathway/KEGG_result/result_kegg")
load("transcriptome_pathway/KEGG_result/result_kegg")
...
# showtext_auto()
# ggsave(plot,
#        filename = "transcriptome_pathway/KEGG_result/kegg_sim_plot.pdf",
#        width = 9,
#        height = 7)


dir.create("transcriptome_pathway/KEGG_result/KEGG_sim_plot")
...
# )
# 
# saveWorkbook(wb, "transcriptome_pathway/KEGG_result/result_kegg_cluster.xlsx", overwrite = TRUE)
...
# reactome_sim_matrix <-
#   tryCatch(
#     expr =  simplifyEnrichment::term_similarity_from_Reactome(term_id = c(transcriptome_crest1_reactome$ID),
#                                                                method = "jaccard") %>%
#       as.data.frame() %>%
#       tibble::rownames_to_column(var = "name1") %>%
#       tidyr::pivot_longer(
...
#   )
# 
# save(reactome_sim_matrix, file = "transcriptome_pathway/Reactome_result/reactome_sim_matrix")

load("transcriptome_pathway/Reactome_result/reactome_sim_matrix")
...
node_data <-
  rbind(transcriptome_crest1_reactome) %>%
  as.data.frame() %>%
  dplyr::select(ID, everything()) %>%
  dplyr::rename(node = ID)
...
#   dplyr::arrange(p.adjust)
# 
# save(result_reactome, file = "transcriptome_pathway/Reactome_result/result_reactome")
load("transcriptome_pathway/Reactome_result/result_reactome")
...
# )
# 
# saveWorkbook(wb, "transcriptome_pathway/Reactome_result/result_reactome.xlsx", overwrite = TRUE)
# 
...
dir.create("transcriptome_pathway/Reactome_result/Reactome_sim_plot")
...
# save(result_all, file = "transcriptome_pathway/result_all")
...
# )
# 
# saveWorkbook(wb, "transcriptome_pathway/result_all.xlsx", overwrite = TRUE)
...
ggsave(plot,
       filename = "transcriptome_pathway/GO_KEGG_Reactome_sim_plot.pdf",
       width = 6,
       height = 7)
