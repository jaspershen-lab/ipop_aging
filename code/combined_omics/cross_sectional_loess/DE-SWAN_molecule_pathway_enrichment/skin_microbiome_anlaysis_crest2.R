...
####skin_microbiome
dir.create("skin_microbiome_pathway_crest2")

####crest 1
temp_data_skin_microbiome_crest2 <-
  temp_data_skin_microbiome %>%
  dplyr::filter(center == 58)

dim(temp_data_skin_microbiome_crest2)
...
addWorksheet(wb, sheetName = "skin_microbiomes in crest2", gridLines = TRUE)
freezePane(wb,
           sheet = 1,
           firstRow = TRUE,
           firstCol = TRUE)
writeDataTable(
  wb,
  sheet = 1,
  x = temp_data_skin_microbiome_crest2 %>% dplyr::select(-Class),
  colNames = TRUE,
  rowNames = FALSE
)
saveWorkbook(
  wb,
  "skin_microbiome_pathway_crest2/skin_microbiome_makrers.xlsx",
  overwrite = TRUE
)
