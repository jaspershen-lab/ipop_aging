...
####gut_microbiome
dir.create("gut_microbiome_pathway_crest1")
...
dim(temp_data_gut_microbiome_crest1)
...
addWorksheet(wb, sheetName = "gut_microbiomes in crest1", gridLines = TRUE)
freezePane(wb,
           sheet = 1,
           firstRow = TRUE,
           firstCol = TRUE)
writeDataTable(
  wb,
  sheet = 1,
  x = temp_data_gut_microbiome_crest1 %>% dplyr::select(-Class),
  colNames = TRUE,
  rowNames = FALSE
)
saveWorkbook(
  wb,
  "gut_microbiome_pathway_crest1/gut_microbiome_makrers.xlsx",
  overwrite = TRUE
)
