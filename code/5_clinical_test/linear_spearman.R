...
dir.create("data_analysis/clinical_test/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("data_analysis/clinical_test/linear_spearman/cross_section")
...
volcano_plot <-
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(
    test_name %in% top10_up_marker_name,
    test_name, NA
  )), size = 3) +
  ggrepel::geom_text_repel(aes(
    label = ifelse(test_name %in% top10_down_marker_name,
                   test_name, NA)
  ), size = 3)
...
pca_object <-
  temp_object %>%
  activate_mass_dataset(what = "variable_info") %>%
  dplyr::filter(test_name %in% aging_markers$test_name) %>%
  massstat::run_pca()
