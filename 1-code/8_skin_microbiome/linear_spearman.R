...
dir.create("3-data_analysis/skin_microbiome/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("3-data_analysis/skin_microbiome/linear_spearman/cross_section")
...
plot <-
  ...
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(n = 11, name = "Spectral"))
...
volcano_plot <-
  ...
  ggrepel::geom_text_repel(aes(
    label = ifelse(Species %in% top10_up_marker_name,
                   Species, NA)
  ), size = 3) +
  ggrepel::geom_text_repel(aes(
    label = ifelse(Species %in% top10_down_marker_name,
                   Species, NA)
  ), size = 3)
...
volcano_plot2 <-
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(
    Species %in% top10_up_marker_name,
    Species, NA
  )), size = 3) +
  ggrepel::geom_text_repel(aes(label = ifelse(
    Species %in% top10_down_marker_name,
    Species, NA
  )), size = 3)
