...
dir.create("3-data_analysis/plasma_metabolomics/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("3-data_analysis/plasma_metabolomics/linear_spearman/cross_section")
...
volcano_plot2 <-
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(
    Compound.name %in% top10_up_marker_name,
    Compound.name, NA
  )), size = 3) +
  ggrepel::geom_text_repel(aes(label = ifelse(
    Compound.name %in% top10_down_marker_name,
    Compound.name, NA
  )), size = 3)
