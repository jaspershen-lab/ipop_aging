...
dir.create("3-data_analysis/plasma_lipidomics/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("3-data_analysis/plasma_lipidomics/linear_spearman/cross_section")
...
volcano_plot <-
  ...
  ggrepel::geom_text_repel(aes(
    label = ifelse(Lipid_Name %in% top10_up_marker_name,
                   Lipid_Name, NA)
  ), size = 3) +
  ggrepel::geom_text_repel(aes(
    label = ifelse(Lipid_Name %in% top10_down_marker_name,
                   Lipid_Name, NA)
  ), size = 3)
...
volcano_plot2 <-
  ...
  ggrepel::geom_text_repel(aes(label = ifelse(
    Lipid_Name %in% top10_up_marker_name,
    Lipid_Name, NA
  )), size = 3) +
  ggrepel::geom_text_repel(aes(label = ifelse(
    Lipid_Name %in% top10_down_marker_name,
    Lipid_Name, NA
  )), size = 3)
