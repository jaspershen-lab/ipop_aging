...
dir.create("3-data_analysis/summary_info")
setwd("3-data_analysis/summary_info")
...
temp %>%
  plyr::dlply(.variables = .(x)) %>%
  lapply(function(x) {
    ir_number <-
      x$n[x$IRIS == "IR"]
    is_number <-
      x$n[x$IRIS == "IS"]
    if (length(ir_number) != 0 &
        length(is_number) != 0) {
      return(is_number / ir_number)
    } else{
      if (length(ir_number) == 0) {
        return(1)
      } else{
