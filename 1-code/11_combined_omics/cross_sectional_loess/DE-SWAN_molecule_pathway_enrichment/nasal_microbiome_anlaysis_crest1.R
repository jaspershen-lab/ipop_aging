...
####nasal_microbiome
dir.create("nasal_microbiome_pathway_crest1")

####crest 1
temp_data_nasal_microbiome_crest1 <-
  temp_data_nasal_microbiome %>%
  dplyr::filter(center == 46)

dim(temp_data_nasal_microbiome_crest1)
...
addWorksheet(wb, sheetName = "nasal_microbiomes in crest1", gridLines = TRUE)
freezePane(wb,
           sheet = 1,
           firstRow = TRUE,
           firstCol = TRUE)
writeDataTable(
  wb,
  sheet = 1,
  x = temp_data_nasal_microbiome_crest1 %>% dplyr::select(-Class),
  colNames = TRUE,
  rowNames = FALSE
)
saveWorkbook(
  wb,
  "nasal_microbiome_pathway_crest1/nasal_microbiome_makrers.xlsx",
  overwrite = TRUE
)
