...
####metabolomics
dir.create("metabolomics_pathway_crest2")
dir.create("metabolomics_pathway_crest2/HMDB_result/")
dir.create("metabolomics_pathway_crest2/KEGG_result/")
...
###HMDB enrichment
query_id <-
  temp_data_metabolomics_crest2$HMDB.ID[!is.na(temp_data_metabolomics_crest2$HMDB.ID)] %>%
  stringr::str_split("\\{\\}") %>%
  unlist() %>%
  unique()
...
# 
# save(metabolomics_crest2_hmdb, file = "metabolomics_pathway_crest2/HMDB_result/metabolomics_crest2_hmdb")
...
query_id <-
  temp_data_metabolomics_crest2$KEGG.ID[!is.na(temp_data_metabolomics_crest2$KEGG.ID)] %>%
  stringr::str_split("\\{\\}") %>%
  unlist() %>%
  unique()
...
#   dplyr::filter(p_value < 0.05)
# 
# save(metabolomics_crest2_kegg, file = "metabolomics_pathway_crest2/kegg_result/metabolomics_crest2_kegg")
...
writeDataTable(
  wb,
  sheet = 1,
  x = metabolomics_crest2_kegg@result,
  colNames = TRUE,
  rowNames = FALSE
)
...
saveWorkbook(
  wb,
  "metabolomics_pathway_crest2/KEGG_result/metabolomics_crest2_kegg.xlsx",
  overwrite = TRUE
)
...
writeDataTable(
  wb,
  sheet = 1,
  x = metabolomics_crest2_hmdb@result,
  colNames = TRUE,
  rowNames = FALSE
)
...
saveWorkbook(
  wb,
  "metabolomics_pathway_crest2/HMDB_result/metabolomics_crest2_hmdb.xlsx",
  overwrite = TRUE
)
...
plot
ggsave(plot,
       filename = "metabolomics_pathway_crest2/KEGG_result/enrichment_pathway.pdf",
       width = 7,
       height = 7)

plot <-
  metpath::enrich_scatter_plot(metabolomics_crest2_kegg,
                               y_axis = "p_value_adjust",
                               y_axis_cutoff = 0.05)
plot
ggsave(plot,
       filename = "metabolomics_pathway_crest2/KEGG_result/enrichment_pathway2.pdf",
       width = 7,
       height = 7)
