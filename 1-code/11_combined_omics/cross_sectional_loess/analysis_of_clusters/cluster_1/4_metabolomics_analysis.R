...
setwd(
  "3-data_analysis/combined_omics/fuzzy_c_means_clustering/cross_section_loess/cluster_1/metabolomics_pathway/"
)
...
###cluster 1
cluster1_metabolomics <-
  final_cluster_info %>%
  dplyr::filter(cluster == 1 & class == "metabolomics")

write.csv(cluster1_metabolomics,
          "cluster1_metabolomics.csv",
          row.names = FALSE)

save(cluster1_metabolomics, file = "cluster1_metabolomics")

cluster1_metabolomics$KEGG.ID
...
save(cluster1_pathway, file = "cluster1_pathway")


write.csv(cluster1_pathway, file = "cluster1_pathway.csv", row.names = FALSE)

stringr::str_split(cluster1_pathway$mapped_id, ";")[[1]] %>% 
