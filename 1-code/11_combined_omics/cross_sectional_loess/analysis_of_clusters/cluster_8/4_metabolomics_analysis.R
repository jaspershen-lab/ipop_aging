...
setwd(
  "3-data_analysis/combined_omics/fuzzy_c_means_clustering/cross_section_loess/cluster_8/metabolomics_pathway/"
)
...
###cluster 8
cluster8_metabolomics <-
  final_cluster_info %>%
  dplyr::filter(cluster == 8 & class == "metabolomics")

write.csv(cluster8_metabolomics,
          "cluster8_metabolomics.csv",
          row.names = FALSE)

save(cluster8_metabolomics, file = "cluster8_metabolomics")

cluster8_metabolomics$KEGG.ID
...
save(cluster8_pathway, file = "cluster8_pathway")

write.csv(cluster8_pathway, file = "cluster8_pathway.csv", row.names = FALSE)

stringr::str_split(cluster8_pathway$mapped_id, ";")[[1]] %>% 
