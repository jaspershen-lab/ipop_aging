...
setwd(
  "3-data_analysis/combined_omics/fuzzy_c_means_clustering/cross_section_loess/cluster_11/metabolomics_pathway/"
)
...
###cluster 11
cluster11_metabolomics <-
  final_cluster_info %>%
  dplyr::filter(cluster == 11 & class == "metabolomics")

write.csv(cluster11_metabolomics,
          "cluster11_metabolomics.csv",
          row.names = FALSE)

save(cluster11_metabolomics, file = "cluster11_metabolomics")

cluster11_metabolomics$KEGG.ID
...
save(cluster11_pathway, file = "cluster11_pathway")

write.csv(cluster11_pathway, file = "cluster11_pathway.csv", row.names = FALSE)

stringr::str_split(cluster11_pathway$mapped_id, ";")[[1]] %>% 
