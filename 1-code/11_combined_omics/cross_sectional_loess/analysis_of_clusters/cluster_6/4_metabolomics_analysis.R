...
setwd(
  "3-data_analysis/combined_omics/fuzzy_c_means_clustering/cross_section_loess/cluster_6/metabolomics_pathway/"
)
...
###cluster 6
cluster6_metabolomics <-
  final_cluster_info %>%
  dplyr::filter(cluster == 6 & class == "metabolomics")

write.csv(cluster6_metabolomics,
          "cluster6_metabolomics.csv",
          row.names = FALSE)

save(cluster6_metabolomics, file = "cluster6_metabolomics")

cluster6_metabolomics$KEGG.ID
...
save(cluster6_pathway, file = "cluster6_pathway")


write.csv(cluster6_pathway, file = "cluster6_pathway.csv", row.names = FALSE)

stringr::str_split(cluster6_pathway$mapped_id, ";")[[1]] %>% 
