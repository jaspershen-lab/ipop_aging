...
dir.create("3-data_analysis/plasma_cytokine/linear_spearman/cross_section/",
           recursive = TRUE)

setwd("3-data_analysis/plasma_cytokine/linear_spearman/cross_section")
...
volcano_plot <-
  ...
  ggrepel::geom_text_repel(aes(
    label = ifelse(SYMBOL %in% top10_up_marker_name,
                   SYMBOL, NA)
  ), size = 3) +
  ggrepel::geom_text_repel(aes(
    label = ifelse(SYMBOL %in% top10_down_marker_name,
                   SYMBOL, NA)
  ), size = 3)
